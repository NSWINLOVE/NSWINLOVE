...
$releaseLines = lines(render_content_with_snippets((string)($release['content'] ?? ''), $snippets));
$requirementsTitle = $content['requirements_title'] ?? '系统要求';
$requirementsLines = lines(render_content_with_snippets((string)($content['requirements'] ?? ''), $snippets));
$guideTitle = $content['guide_title'] ?? '安装教程';
$guideLines = lines(render_content_with_snippets((string)($content['guide'] ?? ''), $snippets));
$faqTitle = $content['faq_title'] ?? '常见问题';
$faqItems = faqPairs(render_content_with_snippets((string)($content['faq'] ?? ''), $snippets));
$noticeEnabled = !empty($notice['enabled']);
$noticeTitle = $notice['title'] ?? '站点公告';
$noticeContent = render_content_with_snippets((string)($notice['content'] ?? ''), $snippets);
...
<div class="wrap">
  ...
  <?php $site['footer_text'] = $displaySettings['footer_text'] ?? ($site['footer_text'] ?? ''); $site['footer_code'] = ''; require __DIR__ . '/system/common-footer.php'; site_footer_render($site); ?>
</div>
